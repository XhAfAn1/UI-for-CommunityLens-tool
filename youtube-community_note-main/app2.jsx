...
const ContentCredentialsCard = ({ isDarkMode }) => {
    return (
        <div className={`mt-6 rounded-2xl overflow-hidden border ${isDarkMode ? 'bg-[#1e1e1e] border-[#3f3f3f]' : 'bg-white border-gray-200'} shadow-sm`}>
            ...
            {/* Metadata Grid */}
            <div className="px-6 py-6 grid grid-cols-1 md:grid-cols-3 gap-y-8 gap-x-12">
                {/* Col 1: CONTENT CREDENTIALS */}
                <div className="flex flex-col gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-500' : 'text-[#909090]'}`}>Content Credentials</span>
                    <div className="flex items-center gap-3">
                        <FileText size={18} className={isDarkMode ? 'text-zinc-400' : 'text-[#606060]'} />
                        <span className={`text-sm font-medium ${isDarkMode ? 'text-zinc-200' : 'text-[#0f0f0f]'}`}>C2PA provenance metadata</span>
                    </div>
                </div>

                {/* Col 2: SIGNED BY */}
                <div className="flex flex-col gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-500' : 'text-[#909090]'}`}>Signed By</span>
                    <div className="flex items-center gap-3">
                        <User size={18} className={isDarkMode ? 'text-zinc-400' : 'text-[#606060]'} />
                        <span className={`text-sm font-medium ${isDarkMode ? 'text-zinc-200' : 'text-[#0f0f0f]'}`}>mady · 9 Mar 2026</span>
                    </div>
                </div>

                {/* Col 3: AI TOOL */}
                <div className="flex flex-col gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-500' : 'text-[#909090]'}`}>AI Tool</span>
                    <div className="flex items-center gap-3">
                        <Bot size={18} className={isDarkMode ? 'text-zinc-400' : 'text-[#606060]'} />
                        <span className={`text-sm font-medium ${isDarkMode ? 'text-zinc-200' : 'text-[#0f0f0f]'}`}>Advanced AI Video Generator</span>
                    </div>
                </div>

                {/* Row 2: PROCESSING */}
                <div className="flex flex-col gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-500' : 'text-[#909090]'}`}>Processing</span>
                    <div className="flex items-center gap-3">
                        <Wand2 size={18} className={isDarkMode ? 'text-zinc-400' : 'text-[#606060]'} />
                        <span className={`text-sm font-medium ${isDarkMode ? 'text-zinc-200' : 'text-[#0f0f0f]'}`}>Synthesized with VFX</span>
                    </div>
                </div>

                {/* Row 2: ACTION HISTORY */}
                <div className="flex flex-col gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-500' : 'text-[#909090]'}`}>Action History</span>
                    <div className="flex items-center gap-3">
                        <List size={18} className={isDarkMode ? 'text-zinc-400' : 'text-[#606060]'} />
                        <span className={`text-sm font-medium ${isDarkMode ? 'text-zinc-200' : 'text-[#0f0f0f]'}`}>Created, Edited, Published</span>
                    </div>
                </div>

                {/* Row 2: ISSUER */}
                <div className="flex flex-col gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? 'text-zinc-500' : 'text-[#909090]'}`}>Issuer</span>
                    <div className="flex items-center gap-3">
                        <Award size={18} className={isDarkMode ? 'text-zinc-400' : 'text-[#606060]'} />
                        <span className={`text-sm font-medium ${isDarkMode ? 'text-zinc-200' : 'text-[#0f0f0f]'}`}>GlobalSign GCC R3</span>
                    </div>
                </div>
            </div>
            ...
            <div className="px-6 pb-6">
                <div className={`p-4 rounded-xl flex gap-3 border ${isDarkMode ? 'bg-[#15201b] border-[#23352c]' : 'bg-[#f4fcf9] border-[#e0f2eb]'}`}>
                    ...
                    <p className={`text-[13px] leading-relaxed ${isDarkMode ? 'text-emerald-100' : 'text-[#1a3a2e]'}`}>
                        <strong className="font-bold">Cryptographically verified.</strong> The digital signature confirms that the associated C2PA provenance metadata has not been tampered with since the content was originally signed.
                    </p>
                </div>
            </div>

            {/* Footer */}
            <div className={`px-6 py-4 border-t flex items-center ${isDarkMode ? 'bg-[#1a1a1a] border-[#3f3f3f]' : 'bg-gray-50 border-gray-100'}`}>
